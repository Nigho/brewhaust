...
Below are some notes on how to build Brewhaust for Windows.

The options known to work for building Brewhaust on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Brewhaust Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Brewhaust applications.
...
Note that for WSL the Brewhaust source path MUST be somewhere in the default mount file system, for
example /usr/src/brewhaust, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\brewhaust`, for example:

    make install DESTDIR=/mnt/c/workspace/brewhaust
...
causes two of the brewhaust executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the brewhaust code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the brewhaust source code).
