...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending BREWHAUSTs\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending BREWHAUSTs\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending BREWHAUSTs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in BREWHAUST/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue callcontract(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 3){
        CTxDestination brewhaustSenderAddress = DecodeDestination(params[2].get_str());
        if(IsValidDestination(brewhaustSenderAddress)) {
            CKeyID *keyid = boost::get<CKeyID>(&brewhaustSenderAddress);
...
UniValue createcontract(const UniValue& params, bool fHelp){
    ...
    uint64_t blockGasLimit = brewhaustDGP.getBlockGasLimit(chainActive.Height());
    uint64_t minGasPrice = CAmount(brewhaustDGP.getMinGasPrice(chainActive.Height()));
    ...
    if (fHelp || params.size() < 1 || params.size() > 6)
        throw runtime_error(
                ...
                "3. gasPrice  (numeric or string, optional) gasPrice BREWHAUST price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
    ...
    if (params.size() > 3){
        ...
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Brewhaust address to send from");
        else
            fHasSender=true;
    }
...
UniValue sendtocontract(const UniValue& params, bool fHelp){
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
                ...
                "5. gasPrice  (numeric or string, optional) gasPrice Brewhaust price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)+"\n"
