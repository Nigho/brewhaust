...
#include <util.h>
#include <main.h>
#include "brewhauststate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

BrewhaustState::BrewhaustState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BrewhaustState::openDB(_path + "/brewhaustDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BrewhaustState::BrewhaustState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BrewhaustState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BrewhaustTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBrewhaustAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            brewhaust::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BrewhaustState::vins() const // temp
{
...
void BrewhaustState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BrewhaustState::vin(dev::Address const& _a) const
{
    return const_cast<BrewhaustState*>(this)->vin(_a);
}

Vin* BrewhaustState::vin(dev::Address const& _addr)
{
...
// void BrewhaustState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BrewhaustState::kill(dev::Address _addr)
{
...
void BrewhaustState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address BrewhaustState::createBrewhaustAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void BrewhaustState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BrewhaustState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BrewhaustState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
