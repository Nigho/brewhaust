#include "brewhaustDGP.h"

void BrewhaustDGP::initDataEIP158(){
...
bool BrewhaustDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule BrewhaustDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t BrewhaustDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BrewhaustDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BrewhaustDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BrewhaustDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BrewhaustDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BrewhaustDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BrewhaustDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BrewhaustDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BrewhaustDGP::createParamsInstance(){
...
dev::Address BrewhaustDGP::getAddressForBlock(unsigned int blockHeight){
...
void BrewhaustDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BrewhaustDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BrewhaustDGP::parseStorageOneUint64(uint64_t& value){
...
void BrewhaustDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BrewhaustDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
