...
bool ToHash160(const std::string& strBrewhaustAddress, std::string& strHash160)
{   
    CTxDestination brewhaustAddress = DecodeDestination(strBrewhaustAddress);
    if(IsValidDestination(brewhaustAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&brewhaustAddress);
...
bool ToBrewhaustAddress(const std::string& strHash160, std::string& strBrewhaustAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strBrewhaustAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBrewhaustAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBrewhaustAddress(tokenEvent.receiver, tokenEvent.receiver);
