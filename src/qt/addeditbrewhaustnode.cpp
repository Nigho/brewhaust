...
#include "ui_addeditbrewhaustnode.h"
...
AddEditBrewhaustNode::AddEditBrewhaustNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditBrewhaustNode)
{
    ui->setupUi(this);

}

AddEditBrewhaustNode::~AddEditBrewhaustNode()
{
    delete ui;
}


void AddEditBrewhaustNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMyBrewhaustNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteBrewhaustNodeConfig(c.sAddress, c);
        uiInterface.NotifyBrewhaustNodeChanged(c);

        accept();
    }
}

void AddEditBrewhaustNode::on_cancelButton_clicked()
{
    reject();
}
