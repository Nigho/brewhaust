...
#include "addeditbrewhaustnode.h"
...
static void NotifyBrewhaustNodeUpdated(MasternodeManager *page, CBrewhaustNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateBrewhaustNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyBrewhaustNodeChanged.connect(boost::bind(&NotifyBrewhaustNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyBrewhaustNodeChanged.disconnect(boost::bind(&NotifyBrewhaustNodeUpdated, this, _1));
}
...
void MasternodeManager::updateBrewhaustNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        BOOST_FOREACH(PAIRTYPE(std::string, CBrewhaustNodeConfig) adrenaline, pwalletMain->mapMyBrewhaustNodes)
        {
            updateBrewhaustNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditBrewhaustNode* aenode = new AddEditBrewhaustNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CBrewhaustNodeConfig c = pwalletMain->mapMyBrewhaustNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    BrewhaustNodeConfigDialog* d = new BrewhaustNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyBrewhaustNodes.erase(sAddress);
        walletdb.EraseBrewhaustNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CBrewhaustNodeConfig) adrenaline, pwalletMain->mapMyBrewhaustNodes)
    {
        CBrewhaustNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
