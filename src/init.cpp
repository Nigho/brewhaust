...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "brewhaust.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "brewhaustd.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " +
                strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 48323, 28333) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in BREWHAUST/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in BREWHAUST/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           brewhaust (or specifically: darksend, instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in BREWHAUST/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -anonymizebrewhaustamount=<n>     " + strprintf(_("Keep N BREWHAUST anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The BREWHAUST-Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that BREWHAUST is running in a usable environment with all
...
#endif
    // Make sure only a single BREWHAUST process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateBrewhaust";
                ...
                boost::filesystem::path brewhaustStateDir = GetDataDir() / "stateBrewhaust";

                bool fStatus = boost::filesystem::exists(brewhaustStateDir);
                const std::string dirBrewhaust(brewhaustStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsBrewhaustState = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<BrewhaustState>(new BrewhaustState(dev::u256(0), BrewhaustState::openDB(dirBrewhaust, hashDB, dev::WithExisting::Trust), dirBrewhaust, existsBrewhaustState));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::brewhaustMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(brewhaustStateDir.string());
...
// ********************************************************* Step 8: load wallet
    ...
    LogPrintf("BREWHAUST darksend rounds %d\n", nDarksendRounds);
    ...
    /* Denominations
       ...
       For example:
       1Brewhaust+1000 == (.1Brewhaust+100)*10
       10Brewhaust+10000 == (1Brewhaust+1000)*10
    */
